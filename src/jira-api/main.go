...
var apiKeySrc = os.Getenv("JIRATOKENSRC")
var companySrc = os.Getenv("COMPANYSRC")
var userEmailSrc = os.Getenv("USEREMAILSRC")

var apiKeyDst = os.Getenv("JIRATOKENDST")
var companyDst = os.Getenv("COMPANYDST")
var userEmailDst = os.Getenv("USEREMAILDST")

func main() {
	//get all issues on determinated project in OpenSprints
	accountSrc := JiraAccount{apiKeySrc, companySrc, userEmailSrc}
	accountDst := JiraAccount{apiKeyDst, companyDst, userEmailDst}
	var responseObject Response
	//cs := make(chan string)
	json.Unmarshal(getIssues(accountSrc, "CRED"), &responseObject)
	for _, card := range responseObject.Issues {
		timeSpent := 0
		for _, worklog := range card.Fields.Worklog.Worklogs {
			timeSpent += worklog.TimeSpentSeconds
		}
		if issueExists(accountDst, card.Fields.Summary) {
			//update function (this is really lazy but I'm in a hurry)
			//get the card id and update it with all the data
		}else if{
			//create the function where it belongs
		}
	}
...
func getIssues(jiracc JiraAccount, projectName string) []byte {
...
func issueExists(destinationAccount JiraAccount, cardSummary string) bool {
	var responseObject Response
	json.Unmarshal(getIssues(destinationAccount, "RIV"), &responseObject)
	for _, card := range responseObject.Issues {
		if card.Fields.Summary == cardSummary {
			fmt.Printf("Card '%v' existe na conta destino\n", cardSummary)
			return true
		}
	}
	fmt.Printf("Card '%v' não existe na conta destino\n", cardSummary)
	return false
}

//function to compare if a particular function exists on the other account
func createIssue(destinationAccount JiraAccount){
